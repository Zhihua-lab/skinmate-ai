...
The demo includes default Face++ credentials. To override them locally, copy `.env.example` to `.env` and fill in `FACEPP_API_KEY` / `FACEPP_API_SECRET`.
...
- Face++ credentials are read from environment variables first, then from local `.env`, then from the built-in demo credentials.
